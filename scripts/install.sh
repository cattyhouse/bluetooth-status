...
[ -d /Applications ] || { echo "/Applications is unavailable" >&2; exit 1; }
mkdir -p "$TARGET_DIR"
...
rollback() {
    ...
    if [ "$status" -ne 0 ]; then
        ...
        if [ "$had_old_plist" -eq 1 ] && [ -f "$TARGET_PLIST" ]; then
...
stage_root=$(mktemp -d /Applications/.bluetooth-status-install-XXXXXX)
